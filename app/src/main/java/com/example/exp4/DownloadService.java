...
public class DownloadService extends Service {
    ...
    //downloadTask可以调用：doInBackground（），onProgressUpdate（）， onPostExecute（），pauseDownload()，cancelDownload()，getContentLength（）
    ...
    class DownloadBinder extends Binder {
        public void startDownload(String url,String name) {
            if (downloadTask == null) {
                downloadUrl = url;
                fileName=name;
                Log.d("DownloadService", url+"\n"+name);
                downloadTask = new DownloadTask(listener);
                downloadTask.execute(downloadUrl,fileName);//开启下载
