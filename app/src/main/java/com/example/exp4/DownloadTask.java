...
public class DownloadTask extends AsyncTask<String, Integer, Integer> {
    //参数1：传一个字符串参数给后台任务；参数2：用来作为进度显示单位；参数3：反馈执行结果
    ...
    //在后台执行具体的下载逻辑
    @Override
    protected Integer doInBackground(String... params) {//从参数中获取下载的url地址
        InputStream is = null;
...
//            String fileName = downloadUrl.substring(downloadUrl.length() - 10);//解析出文件名

            //将文件下载到Environment.DIRECTORY_DOWNLOADS目录下（也是SD卡的download目录）
            String directory = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getPath();
            Log.d("Main DDD", directory);
//            Environment
//            File file = new File(getExternalFilesDir(null), name);

            //判断目录中是否已存在要下载的文件
            file = new File(directory + "/" + fileName);
            Log.d("Main DDD", file.getAbsolutePath());
